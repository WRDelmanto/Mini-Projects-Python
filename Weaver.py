...
def black(y_test, x_test, variable):
    color = [image.item(y_test, x_test, 0), image.item(
        y_test, x_test, 1), image.item(y_test, x_test, 2)]
...
def pixels_analysis(point_1, point_2):
    ...
    for pixel in range(1, step):
        ...
        color = [image.item(pxp[0], pxp[1], 0), image.item(
            pxp[0], pxp[1], 1), image.item(pxp[0], pxp[1], 2)]
...
while True:
    ...
    cv2.line(image, (line[1][0], line[1][1]),
             (line[0][0], line[0][1]), (255, 255, 255), 1)
    cv2.line(cleaned, (line[1][0], line[1][1]),
             (line[0][0], line[0][1]), (0, 0, 0), 1)

    display[0: display.shape[0], 0: int(display.shape[1]/2)] = image
    display[0: display.shape[0], int(
        display.shape[1]/2): display.shape[1]] = cleaned
